...
# تحميل الموديلات
model = YOLO('D:/MODELS/cheating/best.pt')
model_phone = YOLO("D:\MODELS\model_phone/best.pt")
...
def process_frame(frame):
    ...
    for track in device_tracks:
        ...
        hands_result = mp.solutions.hands.Hands(False, max_num_hands=2, min_detection_confidence=0.6, min_tracking_confidence=0.6).process(rgb_frame)############
        hand_points = []
        ...
        if in_hand:
            ...
            elif now - in_hand_start[tid] > 0.5:#########
    ...
    for box in results.boxes:
        ...
        if not class_name:
            continue
        if class_name == "Head-Suspiciousrotation" and conf < 0.5:
            continue
        if class_name == "Hand-Suspiciousrotation" and conf < 0.4:
            continue
    ...
    for track in tracks:
        ...
        for side in ["RIGHT", "LEFT"]:
            ...
            if wrist_xy[1] > table_level_y and abs(wrist_xy[0] - shoulder_xy[0]) < 60:#######
                suspicious = False

            if wrist_xy[1] > shoulder_xy[1] + 20 and abs(wrist_xy[0] - shoulder_xy[0]) < 60:########
                suspicious = False
